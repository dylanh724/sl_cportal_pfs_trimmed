...
# polling Captive Portal for number of concurrent users
CP=`/usr/local/bin/php -q /usr/local/bin/captiveportal_gather_stats.php 'smartlaunch' 'concurrent'`
/usr/bin/nice -n20 /usr/local/bin/rrdtool update /var/db/rrd/captiveportal-smartlaunch-concurrent.rrd ${CP}

# polling Captive Portal for number of logged in users
CP=`/usr/local/bin/php -q /usr/local/bin/captiveportal_gather_stats.php 'smartlaunch' 'loggedin'`
/usr/bin/nice -n20 /usr/local/bin/rrdtool update /var/db/rrd/captiveportal-smartlaunch-loggedin.rrd ${CP}
sleep 60
done
